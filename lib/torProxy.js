...
// Local Tor SOCKS proxy launched by Electron
const localTor = 'socks5://127.0.0.1:9050';

// Get next proxy with Tor-like circuit rotation (random selection for frequent changes)
async function getNextProxy() {
  // Try embedded Tor service launched by Electron
  const torHosts = [
    { host: '127.0.0.1', port: 9050 }  // Embedded Tor SOCKS proxy
  ];

  for (const localTor of torHosts) {
    try {
      // Quick connectivity test
      const testAgent = new SocksProxyAgent(`socks5://${localTor.host}:${localTor.port}`);
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 2000);

      await fetch('https://check.torproject.org/', {
        agent: testAgent,
      ...
      console.log(`🔒 [TOR CIRCUIT] Using Local Tor Service: ${localTor.host}:${localTor.port}`);
      return localTor;
    } catch (error) {
      // Try next host
      continue;
  ...
  console.warn('⚠️ Local Tor service not accessible, falling back to public proxies');

  // Fallback logic (use public proxies if local Tor unavailable)
