...
export class TorIntegration {
  ...
  async getHiddenServiceHostname(): Promise<string | null> {
    try {
      const fs = await import('fs/promises');
      const path = await import('path');

      // Path to the hostname file generated by the Tor service in Docker
      // We assume the app is running in the project root
      const hostnamePath = path.join(process.cwd(), 'monerod', 'data', 'tor-keys', 'hostname');

      try {
        const hostname = await fs.readFile(hostnamePath, 'utf-8');
        return hostname.trim();
      } catch (error) {
        console.warn(`⚠️ Could not read Tor hostname from ${hostnamePath}:`, (error as any).message);
        return null;
      }
    } catch (error) {
      console.error('Failed to get hidden service hostname:', error);
      return null;
    }
  }
}
